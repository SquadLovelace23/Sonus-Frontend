...
const Searchbar = () => {
  ...
  const uniqueArtists: Song[] = Array.from(
    new Set(songs.map((song: Song) => song.Artist[0].name))
  ).map((name) =>
    songs.find((song: Song) => song.Artist[0].name === name)
  ) as unknown as Song[];

  const uniqueAlbums: Song[] = Array.from(
    new Set(songs.map((song: Song) => song.Album[0].name))
  ).map((name) =>
    songs.find((song: Song) => song.Album[0].name === name)
  ) as unknown as Song[];
  ...
  return (
    <main className="card-main">
      ...
      <section className="display-panel-content">
        ...
        {query && <h2 className="artist-title">{t("Users")}</h2>}
        ...
        {query && <h2 className="most-played-title">{t("All songs")}</h2>}
            ...
            songs
              .filter((song: Song) => {
                ...
                if (query) {
                  ...
                  const artistNameToLowerCase =
                    song.Artist[0].name.toLowerCase();
                  return (
                    songNameToLowerCase.includes(query.toLowerCase()) ||
                    artistNameToLowerCase.includes(query.toLowerCase())
                  );
                }
              })
              .map((song: Song) => (
                <div
                  className="most-played"
                  onDoubleClick={() => handleSong(song)}
                >
                  <div
                    className="image-most-played"
        ...
        {query && <h2 className="artist-title">{t("Artists")}</h2>}
        <article className="searchbar-artist-swiper">
          <Swiper
            navigation={true}
            modules={[Navigation]}
                ...
                .map((song: Song) => (
                  <SwiperSlide>
                    <NavLink to={`/artist/${song.artistId}`} className="artist">
                      <div
                        style={{
                          backgroundImage: `url(${song.Artist[0].img})`,
                        }}
                        className="image-artist"
                      />
        ...
        {query && <h2 className="playlist-title">{t("Playlists")}</h2>}
              ...
              playlists
                .filter((playlist: Playlist) => {
                  ...
                  if (query) {
                    ...
                    return playlistNameToLowerCase.includes(
                      query.toLowerCase()
                    );
                  }
                })
        ...
        {query && <h2 className="albums-title">{t("Albums")}</h2>}
            ...
            {uniqueAlbums &&
              uniqueAlbums
                .filter((song: Song) => {
                  if (!query) return;
                  if (query) {
                    const albumNameToLowerCase =
                      song.Album[0].name.toLowerCase();
                    const artistNameToLowerCase =
                      song.Artist[0].name.toLowerCase();
                    return (
                      albumNameToLowerCase.includes(query.toLowerCase()) ||
                      artistNameToLowerCase.includes(query.toLowerCase())
                    );
                  }
                })
