...
import { addSongToPlaylist, deleteSongFromPlaylist, getAllPlaylistsByUserId } from "../../services/playlist.service";
...
const AddSongModal: FC<AddSongModalProps> = ({ song, updateSong }) => {
    ...
    const isCurrentPlaylist = currentUser?.Playlist.some((playlist) => playlist.id === playlistIdFromRoute);
    ...
    const onSubmit = async (selectedPlaylistId: string) => {
        try {
            if (selectedPlaylistId && currentUser) {
                ...
                const songAddedToPlaylist = await addSongToPlaylist(selectedPlaylistId, song?.id, currentUser?.id, token);
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            const isDeleted = await deleteSongFromPlaylist(playlistIdFromRoute, song.id, token);
            if (isDeleted) {
                updateSong();
            } else {
    ...
    return (
        <>
            ...
            />
            <article className={`song-modal ${isElementHidden ? 'song-hidden-modal' : 'song-visible-modal'}`}>
                {playlists.length === 0 ? (
                    ...
                    <>
                        ...
                        {playlists.map((playlist: Playlist) => (
                            <p className="song-modal-titles" key={playlist.id.toString()} onClick={() => onSubmit(playlist.id)}>{playlist?.name}</p>
                        ))}
                    </>
                )}
            </article>
            {isCurrentPlaylist ? (
                ...
                <article className="song-action-wrapper">
                    <svg onClick={toggleElement} xmlns="http://www.w3.org/2000/svg" className="icon icon-tabler icon-tabler-dots" width="24" height="24" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor" fill="none" strokeLinecap="round" strokeLinejoin="round">
                        <path stroke="none" d="M0 0h24v24H0z" fill="none" />
