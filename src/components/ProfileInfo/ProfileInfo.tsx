...
const ProfileInfo = () => {
    ...
    const fetchUserData = async () => {
      ...
      try {
        ...
        if (userDataArray) {
          ...
          const userPlaylistsData = await getAllPlaylistsByUserId(
            userDataArray.id,
    ...
    const checkFollow = async () => {
      try {
        ...
        if (
          ...
          followedUsers.FollowedUser.length > 0
        ) {
          const isFollowed =
            followedUsers.FollowedUser[0].id === currentUser?.id;
  ...
  const handleFollowClick = async () => {
    try {
      ...
      const updatedFollowedUsers = followed
        ? followedUsers - 1
        : followedUsers + 1;
              ...
              {name === user?.name && (
                <article className="camera-container">
                  ...
                  <svg
                    ...
                    className="icon-tabler-camera-plus"
                    width="24"
                    height="24"
                    viewBox="0 0 24 24"
                    strokeWidth="2"
